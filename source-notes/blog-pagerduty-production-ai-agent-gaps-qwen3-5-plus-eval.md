...
> system at enterprise scale. Published June 2026 — foundational framing
> article for the SRE Agent deep-dive that followed.
...
### Claim 1: The prototype-to-production gap for AI agents is structural — demos are easy, reliability is hard
...
- No other sub-pages were followed. The article is self-contained and
  ...
  content to explore.
- Quotes were extracted verbatim from the source via WebFetch. All quotes
  marked as direct are ≤125 characters and were confirmed as verbatim by
  the extraction tool. The Assayer should spot-check key quotes against
  the live URL.
...
- **Eval note**: This is a candidate model evaluation (qwen3.5-plus, zen
  backend) for issue #4. The baseline source note
  (blog-pagerduty-production-ai-agent-gaps.md) was produced by
  DeepSeek/Flash. This eval note independently extracts the same source
  to enable a quality comparison between models.
