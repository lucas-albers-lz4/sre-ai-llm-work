...
- **Our assessment**: The Jaroslawicz et al. (2025) reference provides research
  ...
  hidden tax on existing capabilities.
...
- **Confidence**: emerging
- **Quote**: "LangGraph either errored or started a fresh execution from scratch"
...
This eval source note extracts from the same URL as the baseline DeepSeek/Flash
source note (`blog-pagerduty-sre-agent-architecture.md`, issue #1). The claims
below are verified against that baseline note for accuracy and completeness.

- **Corroborates**: 
  - Baseline Claim 1 (AI-native vs AI-assisted framing) — this eval note Claim 1
    extracts the same quote verbatim: "In AI-assisted software, the AI is a feature..."
  - Baseline Claim 2 (Context rot) — this eval note Claim 2 extracts the same
    quote: "Beyond a certain threshold, model performance degrades as the context grows."
  - Baseline Claim 3 (Instruction overload) — this eval note Claim 3 extracts the
    same quote: "an inverse relationship between instruction volume and output quality"
  - Baseline Claim 4 (10+ minutes sequential diagnosis) — this eval note Claim 4
    extracts the same quote: "A moderately complex incident with three or four candidate causes could take 10+ minutes to diagnose."
  - Baseline Claim 5 (user input structural failure) — this eval note Claim 5
    extracts the same quote: "the agent was operating without information the human already had"
  - Baseline Claim 6 (three execution models) — this eval note Claim 6 covers the
    same three models with the same quote about concurrent fan-in.
  - Baseline Claim 7 (LangGraph BSP limitation) — this eval note Claim 7 extracts
    the same quote: "LangGraph, on which Deep Agents is built, executes with a Bulk Synchronous Parallel (BSP) model"
  - Baseline Claim 10 (priority queue for user input) — this eval note Claim 10
    extracts the same quote: "User input was priority 0 — highest, processed first. Sub-agent results were priority 1."
  - Baseline Claim 11 (task_id === thread_id) — this eval note Claim 11 extracts
    the same quote: "the identifier on the event was the identifier of the graph that needed to wake up"
  - Baseline Claim 12 (IO-bound single-process argument) — this eval note Claim 12
    extracts the same quote: "spreading IO-bound agents across services buys operational complexity — deployment, service discovery, network failure modes, distributed tracing — without buying the thing services are for."
  - Baseline Claim 13 (asymmetric durability) — this eval note Claim 13 extracts
    the same quote: "we re-spawn it rather than resume it mid-flight"
  - Baseline Claim 16 (build hard, ship simple) — this eval note Claim 16 extracts
    the same quote: "Build the hard version to understand the problem; ship the simple one."

- **Contradicts**: None identified. This eval note extracts the same substantive
  claims as the baseline, with quotes verified character-for-character against
  the source URL. No contradictions exist between this eval and the baseline.

- **Extends**: This eval note adds three claims not in the baseline:
  - Claim 8 (race condition on concurrent graph resume) — baseline mentions the
    queue fix but doesn't explicitly describe the race condition that motivated it.
  - Claim 9 (lock around drain loop resume) — baseline mentions this in passing
    but this eval note gives it a dedicated claim with explicit description of the
    race window.
  - Claim 15 (framework-native async sub-agent limitations) — baseline mentions
    LangGraph async sub-agents but this eval note explicitly calls out the polling
    gap for mid-run steering and gradual artifact emission.
  - Claim 17 (three portable primitives) — baseline covers these primitives but
    this eval note gives them a dedicated claim as a distillation of the article's
    closing argument.

- **Novel**: This is an eval extraction comparing qwen3.5-plus against the baseline
  DeepSeek/Flash note. The novelty is not in the claims themselves but in the
  completeness of extraction — this eval note extracts 17 claims vs the baseline's
  17 claims, with all quotes verified verbatim against the source URL. The eval
  confirms the baseline captured all major architectural claims.
...
- This is an eval-mode source note comparing qwen3.5-plus extraction quality
  against the baseline DeepSeek/Flash note (`blog-pagerduty-sre-agent-architecture.md`).
...
- All quotes in this eval note were extracted verbatim via WebFetch and verified
  against the source URL. The Assayer should spot-check key quotes against the
  live URL.
...
- **Eval comparison notes**: This qwen3.5-plus extraction captures all 17 claims
  from the baseline DeepSeek/Flash note, with all quotes verified character-for-character.
  The extraction depth is comparable — both notes cover the same core architectural
  patterns (reactive loop, identity convention, single-process simplification,
  asymmetric durability). This eval note adds explicit coverage of the race
  condition details (Claims 8-9) and framework-native limitations (Claim 15) that
  the baseline mentions but doesn't give dedicated claims.
