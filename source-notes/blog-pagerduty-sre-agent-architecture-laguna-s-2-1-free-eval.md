...
> on LangGraph interrupt/resume primitives, and the counterintuitive
> simplification from distributed to single-process architecture. Published June
> 2026 — very recent, with concrete production patterns.
...
- **Scope**: Covers the full architectural journey — why a single agent failed,
  ...
  "build the hard version to understand, ship the simple one" methodology. Does
  NOT cover: evaluation/accuracy metrics, cost data, specific model choices, or
  failure recovery from model hallucinations.

## Extracted Claims

### Claim 1: AI-native products are fundamentally different from AI-assisted products, and this distinction determines failure modes and engineering trade-offs
...
- **Our assessment**: This is a useful framing that the authors back with concrete
  examples throughout the article. It's a single-source claim but the reasoning is
  sound and the distinction has practical consequences for architecture decisions.
  The claim that failure modes differ materially between the two categories is
...
- **Evidence**: The Incident Context document in their single-agent system grew
  to include JSON blobs of alerts, past incidents, change events, runbook content,
  service topology, dependency graphs, historical patterns, and remediation
  options. The authors cite Liu et al. (2023) "Lost in the Middle" research
  showing that model performance degrades beyond certain context thresholds.
  Newer models are improving but cost and latency impacts remain.
- **Confidence**: settled
...
- **Our assessment**: This is a specific technical claim about LangGraph's
  ...
  interactive agent systems on LangGraph.
...
- **Evidence**: With a regular FIFO queue, if three sub-agent results were
  ...
  ensures user events jump the line.
- **Confidence**: emerging
...
- **Our assessment**: This is the article's most counterintuitive and valuable
  ...
  to compute-bound agent workloads.
...
- **Our assessment**: This asymmetric durability model is a clean design
  ...
  side effects beyond their result).
...
- **Evidence**: The authors evaluated LangGraph's async sub-agents (added after
  ...
  gradual emission of artifacts as each hypothesis resolves."
- **Confidence**: emerging
- **Quote**: "no push notification when a sub-agent finishes, which means no deterministic synthesis the moment each result arrives"
- **Our assessment**: This is an important qualification — framework evolution
  ...
  progressive result emission, which frameworks don't address.
...
- **Confidence**: emerging
- **Quote**: "Understanding why each layer exists is what lets you extend or replace individual pieces as your framework evolves."
- **Our assessment**: This is a useful distillation. The three primitives form a
  ...
  guide design even outside LangGraph.
...
- **Contradicts**: None identified. The PagerDuty companion article covers
  ...
  doesn't cover); this article covers implementation primitives (reactive loop,
  identity, transport, durability model). They are complementary.

- **Extends**:
  - `blog-pagerduty-production-ai-agent-gaps.md` (Claim 6): The companion
    article's "context poisoning" concept — that stale, corrupted, or
    contradictory data in an agent's working memory propagates errors across
  ...
  - `blog-incidentio-ai-sre-incident-run.md` (Claim 3): The incident.io article's
    ...
    pattern at the UX layer.

- **Novel**: The following patterns are new to the corpus and not covered by
  the existing source notes:
  - The three execution models (sequential, parallel-wait-for-all,
    parallel-fan-out-concurrent-fan-in) mapped to SRE investigation requirements
...
- The `miner-related-notes.md` candidates file listed 10 candidates. The top
  candidate (`blog-pagerduty-sre-agent-architecture.md`) is the same URL as this
  source — it is the merged baseline note for issue #1, cited above as the
  same-source reference. The remaining 9 candidates (retail/gaming SRE, SLOs,
  Google AI-SRE, incident response tooling, embracing complexity, reliable
  systems, AI systems at Google, client transparency, and the incident.io AI
  SRE article) were evaluated. Only `blog-incidentio-ai-sre-incident-run.md`
  and `blog-pagerduty-production-ai-agent-gaps.md` were found to corroborate or
  extend specific claims; the other 7 were dismissed as operating at different
  layers (organizational structure, SLO theory, general complexity, etc.) with
  no direct claim overlap.
- Quotes were extracted via WebFetch and spot-checked against the rendered
  page. All quotes marked as direct are short (≤125 chars) and were returned as
...
- **Eval note**: This is a candidate model evaluation extraction (laguna-s-2.1-free
