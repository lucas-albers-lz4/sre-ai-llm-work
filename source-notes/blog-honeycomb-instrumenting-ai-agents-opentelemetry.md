...
- **Corroborates**: None — this is the first source note in the corpus covering OpenTelemetry GenAI semantic conventions for agent instrumentation.
- **Contradicts**: None.
- **Extends**: None directly. This source establishes a new domain of coverage (agent observability with OTel) that future sources can build on.
...
- **OTel spec page**: The OTel GenAI agent spans spec page at `opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-agent-spans/` was checked but returns only a redirect notice — the spec has moved to `github.com/open-telemetry/semantic-conventions-genai`. Not followed further as the Honeycomb docs provide sufficient attribute-level detail for extraction.
- **Quote sourcing**: All quotes in Claims 1-9 and 10-12 were verified against the source URL via targeted WebFetch. Quotes shorter than 125 characters were extracted verbatim. Where no exact quote captures the claim, the paraphrase is noted in `Our assessment`.
