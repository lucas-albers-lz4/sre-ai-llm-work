...
- **Corroborates**:
  ...
  - `docs-langfuse-metrics-overview.md` (#378) **Claim 10** (Data from older SDKs — Python < v4.0.0, JS/TS < v5.0.0 — or OTEL exporters without the `x-langfuse-ingestion-version: 4` header can be delayed by up to 10 minutes on v2 endpoints). This independently documents the same 10-minute delay and ingestion header requirement as this note's Claims 6 and 7, from the metrics API documentation rather than the compatibility page — within-vendor corroboration that strengthens both claims.
  - `docs-langfuse-metrics-overview.md` (#378) **Claim 8** (Legacy Metrics API v1 `GET /api/public/metrics` is deprecated; migration guidance points to Metrics API v2). This directly corroborates this note's Claim 10, which lists `/api/public/metrics` among the deprecated read APIs replaced by Metrics API v2.
...
- **Extends**:
  ...
  - `docs-langfuse-metrics-overview.md` (#378). The metrics note documents the Metrics API v2/v1 endpoint surface, the 10-minute data delay for older SDKs/custom OTel exporters, and the legacy Metrics API v1 deprecation. This compatibility note extends those API-level facts with the full cross-product version matrix (server versions, SDK majors, ingestion paths, evaluators, exports) and the lifecycle policy that governs when each API migration must happen. Together: the metrics note says *what* the APIs and constraints are; this note says *which versions* they apply to and *when* migrations are required.
